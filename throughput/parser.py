...
class Plotter:
    def __init__(self, dir_path, file_blobs=[]):
        ...
        self.results = []
        self.label_mapper = {
            'messageSize': 'messageSize in Bytes',
            'aggregatedEndToEndLatencyAvg': 'aggregatedEndToEndLatencyAvg in ms',
            'aggregatedPublishLatencyAvg': 'aggregatedPublishLatencyAvg in ms',
            'aggregatedPublishLatencyQuantiles': 'aggregatedPublishLatencyQuantiles in ms',
            'aggregatedEndToEndLatencyQuantiles': 'aggregatedEndToEndLatencyQuantiles in ms'
        }
    ...
    def box_plotter(self, x_label, metric="aggregatedEndToEndLatency", title=""):
        ...
        for file_blob in self.results:
            for result in file_blob:
                ...
                box_dict['whislo'] = result.result_dict.get(metric+'Min', 0)
    ...
    def multi_line_percentile_plotter(self, y_label, title=""):
        ...
        plt.ylabel(self.label_mapper.get(y_label, y_label))
